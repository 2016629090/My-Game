Shader "TitleFX_VB"
{
	Properties
	{
		...
		// 添加的新属性
		[Toggle(_SHOWONLY_ON)] _ShowOnly("Show Only (No Disappear)", Float) = 0
	}
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			#pragma shader_feature_local _SHOWONLY_ON
			...
			fixed4 frag(v2f IN  ) : SV_Target
			{
				...
				// 修改动画控制部分
				float Animation221 = 0;
				#ifdef _SHOWONLY_ON
				// 只有出现效果 - 动画值从0到1，然后保持在1
				if (_AutoManualAnimation > 0.5)
				{
					// 手动模式
					Animation221 = clamp(_Animation_Factor, 0.0, 1.0);
				}
				else
				{
					// 自动模式 - 使用时间控制，只出现一次
					float time = _Time.y;
					Animation221 = smoothstep(0.0, 1.0, clamp(time, 0.0, 1.0));
				}
				#else
				// 原来的动画控制（有出现和消失）
				Animation221 = (( _AutoManualAnimation )?( _Animation_Factor ):( _SinTime.w ));
				#endif
				...
				// 在ShowOnly模式下确保最终结果不会消失
				#ifdef _SHOWONLY_ON
				temp_output_46_0 = max(temp_output_46_0, 0.01); // 保持最小可见度
				#endif
